...
for ii = 1 : nBands
    ...
    %tstillinger - deal with uneven size inputs -MAKE A FUNCTION
    tempI= interp23tap(t,ratio);
    
    szI=size(tempI);
    szP=size(PAN_LP);
     if szI(1)~=szP(1)||szI(2)~=szP(2)     
            if szI(1)>szP(1)
                tempI(szP(1)+1:szI(1),:,:)=[];
            elseif szI(1)<szP(1)
                tempI(szI(1)+1:szP(1),:,:)=tempI(szI(1),:,:);
            end
            
            if szI(2)>szP(2)
                tempI(:,szP(2)+1:szI(2),:)=[];
            elseif szI(2)<szP(2)
                tempI(:,szI(2)+1:szP(2),:)=tempI(:,szI(2),:);
            end
     end   
